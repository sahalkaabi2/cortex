import axios from 'axios';
import { LLMProvider } from './base';
import { LLMResponse, MarketData, PortfolioState } from '../types';
import { createTradingPrompt } from './base';
import { calculateLLMCost } from '../costs';

export class QwenProvider implements LLMProvider {
  name = 'Qwen';
  private apiKey: string;
  private apiUrl: string;
  private model: string;

  constructor(model: string = 'qwen-plus') {
    this.apiKey = process.env.QWEN_API_KEY || '';
    this.model = model;

    // Support both International (Singapore) and China (Beijing) regions
    // Default to International region if not specified
    const region = process.env.QWEN_REGION || 'international';

    if (region === 'china' || region === 'beijing') {
      // China (Beijing) region
      this.apiUrl = 'https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions';
    } else {
      // International (Singapore) region - DEFAULT
      this.apiUrl = 'https://dashscope-intl.aliyuncs.com/compatible-mode/v1/chat/completions';
    }

    console.log(`[QWEN] Initialized with model: ${model}, region: ${region}`);
  }

  async makeDecision(
    marketData: MarketData[],
    portfolioState: PortfolioState,
    traderId?: string
  ): Promise<LLMResponse> {
    try {
      const prompt = await createTradingPrompt(marketData, portfolioState, traderId);

      // Log full prompt
      console.log('═══════════════════════════════════════════════════');
      console.log(`[QWEN] Sending prompt at ${new Date().toLocaleTimeString()}`);
      console.log('═══════════════════════════════════════════════════');
      console.log(prompt);
      console.log('═══════════════════════════════════════════════════');

      const response = await axios.post(
        this.apiUrl,
        {
          model: this.model,
          messages: [
            {
              role: 'system',
              content: 'You are an expert cryptocurrency trader with a systematic, data-driven approach. Analyze market conditions rigorously and always provide structured exit plans for your trades. Respond with valid JSON only.',
            },
            {
              role: 'user',
              content: prompt,
            },
          ],
          temperature: 0.7,
        },
        {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${this.apiKey}`,
          },
        }
      );

      const content = response.data.choices[0].message.content;

      // Log raw response
      console.log('───────────────────────────────────────────────────');
      console.log(`[QWEN] Received response at ${new Date().toLocaleTimeString()}`);
      console.log('───────────────────────────────────────────────────');
      console.log(content);
      console.log('═══════════════════════════════════════════════════\n');

      // Extract JSON from response
      const jsonMatch = content.match(/\{[\s\S]*\}/);
      if (!jsonMatch) {
        throw new Error('No JSON found in Qwen response');
      }

      const decision = JSON.parse(jsonMatch[0]);

      // Extract usage and calculate actual cost
      const usage = response.data.usage ? {
        prompt_tokens: response.data.usage.prompt_tokens,
        completion_tokens: response.data.usage.completion_tokens,
        total_tokens: response.data.usage.total_tokens,
      } : undefined;

      const calculated_cost = usage ? calculateLLMCost(this.model, usage) : 0;

      console.log(`[QWEN] Model: ${this.model}, Token usage: ${usage?.total_tokens || 0} tokens, Cost: $${calculated_cost.toFixed(4)}`);

      return {
        action: decision.action,
        coin: decision.coin,
        amount: decision.amount,
        reasoning: decision.reasoning,
        confidence: decision.confidence,
        profit_target: decision.profit_target,
        stop_loss: decision.stop_loss,
        invalidation_condition: decision.invalidation_condition,
        risk_usd: decision.risk_usd,
        prompt_text: prompt,
        raw_response: content,
        usage,
        calculated_cost,
      };
    } catch (error) {
      console.error('Qwen decision error:', error);
      return {
        action: 'HOLD',
        reasoning: `Error making decision: ${error}`,
        confidence: 0,
      };
    }
  }
}
